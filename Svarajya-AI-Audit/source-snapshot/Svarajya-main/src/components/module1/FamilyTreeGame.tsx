...
interface FamilyTreeProps {
    ...
    onSealMandal?: () => void | Promise<void>;
    isSaving?: boolean;
}
...
export const FamilyTreeGame = React.memo(function FamilyTreeGame({ members, onAddMember, onRemoveMember, onEditMember, onSealMandal, isSaving }: FamilyTreeProps) {
    ...
    /* FIX (Bug 8 Validation): Replaced restrictive domain matching bounds with safe universal regex checker */
    ...
    const handleSubmit = useCallback((e: React.FormEvent) => {
        ...
        /* FIX (Bug 8 Submit Handler): Applied identical global validator framework context to handle submit block */
        if (formData.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) {
            setEmailError("Please enter a valid email address");
            return;
        }
                    ...
                    <motion.div key="add-btn" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                        {members.length < 5 && (
                            <button
                                ...
                                className="w-full border border-dashed border-[var(--color-rajya-accent-dim)] hover:border-[var(--color-rajya-accent)] text-[var(--color-rajya-muted)] hover:text-[var(--color-rajya-accent)] py-4 rounded-2xl flex items-center justify-center gap-2 transition-colors group bg-white/5"
                            >
                    ...
                    >
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="space-y-1">
                                    {/* FIX (Bug 8 UI Placeholder): Cleaned layout constraint text to display standard placeholder format context */}
                                    <input
                                        type="email"
                                        placeholder="Email ID (e.g., name@domain.com)"
                            ...
                            {errorMsg && (
                                <p className="text-[var(--color-rajya-danger)] text-xs text-center">
                                    {errorMsg}
                                </p>
                            )}
