...
export default function ItrPage() {
  ...
  const getFileUrl = (fileRef?: string | null): string | undefined => {
    if (!fileRef) return undefined;
    if (String(fileRef).startsWith("http")) {
      return String(fileRef);
    }
    return `/api/vault/download/${fileRef}`;
  };
  ...
  async function fetchRecords() {
    ...
    try {
      ...
      console.log("ITR RECORD", responseData[0]);
                ...
                <div>
                  ...
                  {r.documentUrl || (r as any).documentId ?
                    <button
                      ...
                      onClick={async () => {
                        const fileId = r.documentUrl || (r as any).documentId;
                        if (!fileId) return;

                        const url = await Vault.getPreviewUrl(fileId);
