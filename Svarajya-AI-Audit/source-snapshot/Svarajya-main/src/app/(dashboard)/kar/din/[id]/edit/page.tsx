...
export default function EditDinPage() {
  ...
  const errors = useMemo(() => {
    ...
    } else if (!/^\d{8}$/.test(dinNumber)) {
      fieldErrors.dinNumber = "DIN number must be exactly 8 digits.";
    }
    ...
    } else if (isPastDate(form.expiryDate)) {
    ...
    } else if (isPastDate(form.dscExpiryDate)) {
      fieldErrors.dscExpiryDate = "DSC expiry date cannot be in the past.";
    }
    ...
    } else if (isFutureDate(form.directorSince)) {
      fieldErrors.directorSince = "Director since date cannot be in the future.";
    }
