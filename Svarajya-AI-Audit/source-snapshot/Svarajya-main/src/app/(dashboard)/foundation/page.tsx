...
import { ArrowLeft, Users, GraduationCap, Camera, Trash2, Edit2, Info } from "lucide-react";
...
export default function FoundationHub() {
            ...
            try {
                ...
                if (profileData) {
                    ...
                    if (profileData.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(profileData.email)) {
                        console.warn("Stored email does not match standard global format structure.");
                    }
    ...
    return (
        <div className="flex flex-col min-h-screen p-6 pb-24 relative max-w-5xl mx-auto w-full">
            <div className="absolute inset-0 bg-rajya-card/50 border border-rajya-accent-dim rounded-2xl pointer-events-none -z-10 shadow-lg" />

            <div className="relative z-10 flex flex-col min-h-screen">
                
                {/* FIX (Layout Consistency): Added standalone page title layout header matching Kunji Vault layout matrix */}
                <div className="flex items-center gap-3 pt-8 mb-6">
                    <button onClick={() => router.push("/rajya")} className="w-9 h-9 rounded-xl bg-white/6 border border-white/10 flex items-center justify-center hover:bg-white/10 transition-colors shrink-0">
                        <ArrowLeft className="w-4 h-4 text-white/60" />
                    </button>
                    <div>
                        <h1 className="text-xl font-bold text-white">Sthapana</h1>
                        <p className="text-xs text-white/40">Your credential and access management vault.</p>
                    </div>
                </div>

                {/* FIX (Alert Layout Alignment): Reconstructed custom Info Alert Card with unified design alignment */}
                <div className="bg-white/5 border border-white/10 rounded-2xl p-5 mb-6">
                    <div className="flex items-start gap-3">
                        <Info className="w-5 h-5 text-amber-400 shrink-0 mt-0.5" />
                        <div>
                            <h3 className="text-sm font-semibold text-white">What is Sthapana?</h3>
                            <p className="text-xs text-white/40 mt-1">
                                Complete your personal profile, add family members, and record your education. This forms the base of your financial kingdom.
                            </p>
                            <div className="flex flex-wrap gap-2 mt-3">
                                <span className="inline-flex items-center gap-1 bg-white/5 border border-white/10 px-2.5 py-1 rounded-full text-[11px] text-white/70">
                                    👤 Profile
                                </span>
                                <span className="inline-flex items-center gap-1 bg-white/5 border border-white/10 px-2.5 py-1 rounded-full text-[11px] text-white/70">
                                    👪 Family
                ...
                {/* Single Primary Action Routing Layout Configuration Container */}
