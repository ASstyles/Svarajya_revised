  ...
  return (
    <div className="flex flex-col min-h-screen p-6 pb-24 relative">
      ...
      <div className="relative z-10">
        ...
        {/* Tutorial strip */}
        <div className="bg-rajya-accent/8 border border-rajya-accent/20 rounded-xl p-3 mb-3">
          <p className="text-xs text-rajya-muted">
            💡 <strong className="text-rajya-text">Small leaks</strong> begin
            with forgotten logins.
          </p>
        </div>
        <VideoTutorialPlaceholder
          youtubeId="iWsQY6Ha4OE"
          label="Detecting subscription leakage"
        />
        <div className="h-4" />
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          <div className="bg-white/3 border border-white/8 rounded-xl p-3 text-center">
            ...
            <p className="text-lg font-bold text-white/70">
              {monthlyTotal > 0 ? formatRupee(monthlyTotal) : "—"}
            </p>
          </div>
          <div
            className={`bg-white/3 border rounded-xl p-3 text-center ${annualLeakage > 0 ? "border-red-500/20" : "border-white/8"}`}
          >
            <p className="text-[10px] text-white/30">Annual Leakage Est.</p>
            <p
              className={`text-lg font-bold ${annualLeakage > 0 ? "text-red-400" : "text-white/70"}`}
            >
        ...
        {dormant.length > 0 && (
          <div className="bg-amber-400/10 border border-amber-400/20 rounded-xl p-3 mb-4 flex items-start gap-2">
            ...
            <p className="text-xs text-amber-400">
              {dormant.length} subscription{dormant.length > 1 ? "s" : ""}{" "}
              unused for 90+ days. Consider cancelling.
            </p>
          </div>
        )}
        ...
        {subs.length === 0 && !showAdd && (
          <div className="text-center py-12 mb-4">
            ...
            <p className="text-xs text-white/30 mb-5">
              Add subscriptions to detect silent leaks.
            </p>
          </div>
        )}
        ...
        <div className="space-y-2 mb-5">
          {subs.map((sub) => (
            <div
              key={sub.id}
              className={`bg-white/3 border rounded-xl p-3 ${isDormant(sub.id) ? "border-red-500/20" : "border-white/8"}`}
            >
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-[10px] text-white/30">
                ...
                {sub.autoDebitBank && (
                  <span>Auto-debit: {sub.autoDebitBank}</span>
                )}
              </div>
          ...
          <div className="bg-white/5 border border-white/15 rounded-xl p-4 mb-5 space-y-3">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <span className="absolute left-2 top-1/2 -translate-y-1/2 text-xs text-white/30">
                  ₹
                </span>
                <input
                  type="number"
            ...
            <div className="flex gap-2">
              <button
                onClick={handleSave}
                className="flex-1 bg-amber-400 text-black font-semibold py-2.5 rounded-lg text-xs"
              >
                Save
              </button>
              <button
                ...
                className="px-4 py-2.5 bg-white/5 border border-white/10 rounded-lg text-xs text-white/40"
              >
                Cancel
              </button>
            </div>
