...
export default function WillPage() {
  ...
  const [executorContactError, setExecutorContactError] = useState("");
  ...
  const saveWill = async () => {
    if (!executorName.trim()) {
      toast('Executor name is required.', 'error');
      return;
    }

    const contact = executorContact.trim();

    if (!contact) {
      setExecutorContactError("Executor contact is required.");
      return;
    }

    if (!/^\d{10}$/.test(contact)) {
      setExecutorContactError("Executor contact must be a valid 10-digit phone number.");
      return;
    }
