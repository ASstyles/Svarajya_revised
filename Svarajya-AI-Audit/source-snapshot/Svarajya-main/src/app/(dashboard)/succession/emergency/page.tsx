...
export default function EmergencyPage() {
  ...
  const sendOtp = async () => {
    if (!data?.emergencyContactPhone) {
      setPrimaryPhoneError("Phone number must be a valid 10-digit number.");
      toast('Enter emergency contact phone before sending OTP.', 'error');
      return;
    }

    if (!/^\d{10}$/.test(data.emergencyContactPhone.trim())) {
      setPrimaryPhoneError("Phone number must be a valid 10-digit number.");
      return;
    }
  ...
  const saveEmergency = async () => {
    ...
    const primaryPhone = data.emergencyContactPhone.trim();
    const secondaryPhone = data.secondaryContactPhone?.trim() || "";
    ...
    if (secondaryPhone && !/^\d{10}$/.test(secondaryPhone)) {
      setSecondaryPhoneError("Phone number must be a valid 10-digit number.");
      return;
    }
              ...
              <div className="grid gap-4">
                ...
                <label className="flex flex-col gap-2 text-sm text-white/70">
                  ...
                  <input
                    ...
                    onChange={e => {
                      setField('emergencyContactPhone', formatPhoneInput(e.target.value));
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <label className="flex flex-col gap-2 text-sm text-white/70">
                    ...
                    <input
                      ...
                      onChange={e => {
                        setField('secondaryContactPhone', formatPhoneInput(e.target.value));
