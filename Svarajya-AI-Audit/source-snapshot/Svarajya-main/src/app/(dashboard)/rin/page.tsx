...
export default function RinDashboard() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-rajya-bg flex items-center justify-center">
    ...
    <main className="min-h-screen pb-32 font-sans selection:bg-amber-500/30">
      ...
      <header className="px-6 pt-12 pb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-bold bg-linear-to-r from-amber-300 to-orange-300 bg-clip-text text-transparent">
              Rin (Debt)
            </h1>
            <p className="text-sm text-white/50">
              Measure your chains. Plan your liberation.
            </p>
          </div>
        </div>
      </header>
      ...
      <div className="px-6 space-y-6">
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-6 relative overflow-hidden backdrop-blur-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
              ...
              <div className="flex items-center gap-6">
                <div>
                  ...
                  <p className="text-lg font-bold text-white">
                    ₹{metrics?.totalEMI.toLocaleString("en-IN") || 0}
                  </p>
                </div>
                <div>
                  <p className="text-[10px] text-white/40 uppercase tracking-widest font-semibold mb-1">
            ...
            <div className="flex flex-col justify-center items-center md:items-end">
              <div
                className={`relative w-24 h-24 flex items-center justify-center rounded-full border-4 ${isOverBurdened ? "border-red-500/30" : "border-amber-500/30"}`}
              >
                <div className="text-center">
                  <p
                    className={`text-xl font-bold ${isOverBurdened ? "text-red-400" : "text-amber-400"}`}
                  >
                    {emiBurden.toFixed(1)}%
                  </p>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Link
            href="/rin/loans/add"
            className="bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col items-center justify-center gap-2 hover:bg-white/10 transition-colors"
          >
          ...
          {loans.length === 0 ?
            <div className="text-center py-12 bg-white/5 border border-white/10 border-dashed rounded-3xl">
              ...
              <p className="text-sm text-white/40 font-medium px-8 leading-relaxed">
                No active loans recorded. A debt-free life is the ultimate
                Rajya.
              </p>
            </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-bold text-white">
                          {loan.lenderName || loan.type}
                        </p>
                        <p className="text-[10px] text-white/40 uppercase tracking-widest mt-0.5">
                          {loan.type} • EMI: ₹{loan.emi.toLocaleString("en-IN")}
                        </p>
                      </div>
                    </div>
        ...
        {metrics && metrics.totalEMI > 0 && (
          <div className="p-5 bg-linear-to-br from-indigo-500/10 to-purple-500/10 border border-indigo-500/20 rounded-3xl">
            <div className="flex items-start gap-4">
              <div className="w-10 h-10 rounded-xl bg-indigo-500/20 flex items-center justify-center shrink-0">
                <ArrowUpRight className="w-5 h-5 text-indigo-400" />
              </div>
              <div>
                <h4 className="font-bold text-indigo-100 text-sm">
                  Strategic Prepayment Advice
                </h4>
                <p className="text-xs text-indigo-200/60 mt-1 leading-relaxed">
                  Increasing your total monthly EMI by just{" "}
                  <span className="text-indigo-300 font-bold">10%</span> could
                  save you approximately{" "}
                  <span className="text-indigo-300 font-bold">
                    ₹{(metrics.totalOutstanding * 0.15).toLocaleString("en-IN")}
                  </span>{" "}
                  in total interest over the next 5 years.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="mt-12 px-6 flex items-start gap-3 opacity-50 justify-center text-center pb-8">
        ...
        <p className="text-[10px] text-white/60 leading-relaxed max-w-62.5">
          Svarajya calculations are estimates. Consult a financial advisor for
          specific debt restructuring.
        </p>
      </div>
    </main>
  );
