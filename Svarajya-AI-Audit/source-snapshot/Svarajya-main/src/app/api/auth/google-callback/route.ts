...
export async function GET(request: NextRequest) {
  try {
    ...
    const redirectUri = process.env.GOOGLE_REDIRECT_URI || `${process.env.NEXT_PUBLIC_SITE_URL || 'https://svarajya.com'}/api/auth/google-callback`;
    ...
    if (action === 'link') {
      ...
      const { data: linkData, error: linkError } = await supabaseAdmin.auth.admin.generateLink({
        ...
        email: email,
        options: {
          redirectTo: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://svarajya.com'}/start?success=${isNewUser ? 'Google_Signup' : 'Google_Login'}`,
        },
      });
