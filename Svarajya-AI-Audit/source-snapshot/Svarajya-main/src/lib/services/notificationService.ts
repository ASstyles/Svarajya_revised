...
> {
  ...
  async triggerAutomaticAlerts(userId: string): Promise<void> {
    try {
      ...
      for (const policy of policies) {
        if (
          policy.dueDate &&
          new Date(policy.dueDate) <= thirtyDaysFromNow &&
      ...
      if (
        ...
        typeof budgetPlan.categories === "object"
      ) {
      ...
      for (const sub of subscriptions) {
        if (
          sub.renewalDate &&
          new Date(sub.renewalDate) <= sevenDaysFromNow &&
          new Date(sub.renewalDate) >= now
        ) {
      ...
      if (taxSubject) {
        const exists = await prisma.notification.findFirst({
          where: { userId, subject: taxSubject },
        });
        if (!exists) {
          await prisma.notification.create({
            data: {
      ...
      for (const goal of goals) {
        if (goal.targetAmount && goal.targetAmount > 0) {
          ...
          if (goal.targetDate) {
            ...
            const goalDiffDays = Math.ceil(
              goalDiffTime / (1000 * 60 * 60 * 24),
            );
      ...
      if (user) {
        const isIncomplete =
          !user.name || !user.dob || (!user.phone && !user.primaryMobile);
        if (isIncomplete) {
      ...
      for (const stream of incomeStreams) {
        const lastReview =
          stream.lastReviewedAt ?
            new Date(stream.lastReviewedAt)
      ...
      console.error(
        "[NotificationService] Error triggering automatic alerts:",
        error,
      );
    }
  }
  ...
  getLinkForNotification(subject: string | null): string {
    ...
    if (
      ...
      title.toLowerCase().includes("profile setup incomplete") ||
      title.toLowerCase().includes("profile incomplete")
    ) {
      return "/foundation";
    } else if (
      title.toLowerCase().includes("family member review") ||
      title.toLowerCase().includes("no family members")
    ) {
      ...
      title.toLowerCase().includes("bhoomi")
    ) {
      return "/bhoomi";
    } else if (
  ...
  async markAsRead(id: string, userId: string): Promise<any> {
    try {
      ...
      return {
        ...
        link:
          notification.link ||
          this.getLinkForNotification(notification.subject),
      };
    } catch (error) {
      console.error(
